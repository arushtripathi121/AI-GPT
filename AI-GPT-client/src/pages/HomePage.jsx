...
const HomePage = () => {
  ...
  const fetchDataById = async (id) => {
    if (id) {
      setIsLoading(true);
      try {
        const response = await fetch('https://ai-gpt-server.vercel.app/api/v1/gpt/getChats', {
          method: 'POST',
  ...
  const askNewQuestion = async (prompt) => {
    ...
    try {
      const response = await fetch('https://ai-gpt-server.vercel.app/api/v1/gpt/prompt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body,
      });
