...
const Sidebar = ({ closeMenu, email, conversationId, startNewChat }) => {
    ...
    const fetchConversation = async (email) => {
        setIsLoading(true);
        const response = await fetch('https://ai-gpt-server.vercel.app/api/v1/gpt/getConversationIds', {
            method: 'POST',
            headers: {
    ...
    const deleteConversation = async (conversationId) => {
        setIsLoading(true);
        const response = await fetch('https://ai-gpt-server.vercel.app/api/v1/gpt/deleteConversation', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
